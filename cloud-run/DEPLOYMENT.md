...
curl -fsS -X POST SERVICE_URL/api/agent/run \
  ...
  -d '{"scenario":"dairy_shg","intervention":"timing","consent":{"financial":true,"market":true,"climate":true,"revoked":false},"actor":{"id":"demo-maker","role":"maker"}}'
```
...
Reproduce the refusal paths that protect consent, identity boundaries and runtime safety:

```bash
# withdrawn consent -> 422 before any tool runs
curl -s -X POST SERVICE_URL/api/agent/run -H 'Content-Type: application/json' \
  -d '{"scenario":"dairy_shg","consent":{"financial":true,"revoked":true},"actor":{"id":"demo-maker","role":"maker"}}'
...
- Keep `min-instances=0` and `max-instances=1` for the public demo.
...
- Delete the service and unneeded container images when the public demonstration is no longer required.
